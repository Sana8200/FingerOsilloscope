 ...
 */
void spi_setup(void) {
    // We use standard 'unsigned int' for the mask, as this is typically 32 bits.
    volatile unsigned int direction_mask = 0;

    // --- Configure Outputs ---
    // SCK, DIN, CS, and RST are controlled by the MCU (Set bits to '1').
    direction_mask = SPI_SCK_PIN | SPI_DIN_PIN | ADC_CS_PIN | ADC_RST_PIN;
    *GPIO_PORT_DIRECTION |= direction_mask; 

    // --- Configure Inputs ---
    // DOUT (MISO) and DRDY are signals coming from the ADC (Clear bits to '0').
    // NOTE: SPI_DOUT_PIN is used here, NOT the DOUT_READ() macro.
...
unsigned char spi_transfer(unsigned char tx_byte) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        SCK_LOW();
        // --- TIMING FIX: Greatly increase delay to slow down the clock ---
        delay(5000); 
